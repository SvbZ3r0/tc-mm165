...
public class RecordableGraphics extends Graphics2D {
    ...
    private static final int DRAW_RECORDABLE_GRAPHIC = 57;
    ...
    public RecordableGraphics(BufferedImage img) {
        this.img = img;
        this.g = img.createGraphics();
        this.dict = new HashMap<Object, Integer>();
        this.rev = new HashMap<Integer, Object>();
        this.uid = new AtomicInteger();
        this.replaySeq = new ArrayList<Integer>();
    }

    public void replay(int first, int last) {
        int[] seq = new int[last - first];
        for (int i =0; i < seq.length; i++) seq[i] = this.replaySeq.get(first + i);
        replay(seq);
    }
    public void replay(int[] seq) {
        for (int i = 0; i < seq.length; i++) {
            ...
            case DRAW_RECORDABLE_GRAPHIC:
                RecordableGraphics rg = (RecordableGraphics)p(seq[++i]);
                rg.replay((Integer) p(seq[++i]), (Integer) p(seq[++i]));
                break;
    ...
    public void drawRecordableGraphics(RecordableGraphics g, int x, int y, int w, int h) {
        int lid = g.lastId;
        int nid = g.newFrame();
        addToReplay(DRAW_RECORDABLE_GRAPHIC, g, lid, nid);
    }
    ...
    public int newFrame() {
        this.lastId = this.replaySeq.size();
        return this.lastId;
    }
}
