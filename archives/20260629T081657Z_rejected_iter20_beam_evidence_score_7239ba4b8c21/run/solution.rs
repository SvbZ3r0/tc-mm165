...
fn build_beam_placements(
    ...
    for raw in PRECOMPUTED_PLACEMENTS.iter() {
...
fn beam_fleet_posterior(
    ...
    for len in 1..remaining.len() {
        if remaining[len] > 0 {
            by_len[len] = build_beam_placements(n, len, grid, shot, scans, heat);
    ...
    let words = (n * n + 63) / 64;
    let total_ship_cells: usize = ship_lengths.iter().sum();
    let total_known_hits = known_hits.len();
    ...
    let beam_width = if known_hits.is_empty() { 180usize } else { 260usize };
    let per_len_limit = if known_hits.is_empty() { 260usize } else { 420usize };
    ...
    for &len in &ship_lengths {
        ...
        for state in &states {
            for placement in by_len[len].iter().take(per_len_limit) {
                ...
                let placed_cells = state.placed_cells + placement.len;
                let remaining_after = total_ship_cells.saturating_sub(placed_cells);
                let mut scan_penalty = 0.0;
                for idx in 0..scans.len() {
                    scan_counts[idx] += placement.scan_counts[idx];
                    let need = scans[idx].count.saturating_sub(dead_in_scan[idx]);
                    if scan_counts[idx] > need || scan_counts[idx] + remaining_after < need {
                        ok = false;
                        break;
                    }
                    let expected = if total_ship_cells > 0 {
                        need as f64 * placed_cells as f64 / total_ship_cells as f64
                    } else {
                        need as f64
                    };
                    scan_penalty += (scan_counts[idx] as f64 - expected).abs();
                }
                if !ok {
                    continue;
                }
                let hit_count = state.hit_count + placement.hit_count;
                let remaining_hits = total_known_hits.saturating_sub(hit_count);
                if remaining_hits > remaining_after {
                    continue;
                }
                let mut bits = state.bits.clone();
                bit_or_into(&mut bits, &placement.bits);
                let evidence_score = placement.heat_score * 0.08
                    + placement.hit_count as f64 * 2500.0
                    - scan_penalty * 18.0
                    + placement.prior;
                next_states.push(BeamState {
                    ...
                    hit_count,
                    placed_cells,
                    score: state.score + evidence_score,
                });
            }
        }
...
fn best_hunt_cell(
    ...
    let beam_post = beam_fleet_posterior(n, grid, shot, remaining, scans, remaining_cells, &prob);
    let max_prob = prob
        .iter()
