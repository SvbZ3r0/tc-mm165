...
fn placement_scan_feasibility_weight(
    grid: &[Vec<Cell>],
    r: usize,
    c: usize,
    dr: usize,
    dc: usize,
    len: usize,
    constraints: &[(Scan, usize, usize)],
) -> f64 {
    let mut weight = 1.0f64;
    for &(scan, scan_remaining, scan_unknown) in constraints {
        if scan_unknown == 0 {
            continue;
        }

        let mut overlap = 0usize;
        let mut unknown_overlap = 0usize;
        for k in 0..len {
            let rr = r + dr * k;
            let cc = c + dc * k;
            if scan.contains(rr, cc) {
                overlap += 1;
                if grid[rr][cc] == Cell::Unknown {
                    unknown_overlap += 1;
                }
            }
        }
        ...
        let after_remaining = scan_remaining as isize - overlap as isize;
        let after_unknown = scan_unknown as isize - unknown_overlap as isize;
        if after_remaining < 0 {
            let excess = (-after_remaining).min(4) as i32;
            weight *= 0.25f64.powi(excess);
            continue;
        }
        if after_remaining > after_unknown {
            let shortage = (after_remaining - after_unknown).min(4) as i32;
            weight *= 0.20f64.powi(shortage);
            continue;
        }

        let before_density = scan_remaining as f64 / scan_unknown as f64;
        let overlap_fraction = overlap as f64 / len as f64;
        weight *= 1.0 + 0.35 * before_density.clamp(0.0, 1.0) * overlap_fraction;
    }

    weight.clamp(0.10, 4.00)
}

fn build_scan_feasibility_probabilities(
    n: usize,
    grid: &[Vec<Cell>],
    shot: &[Vec<bool>],
    remaining: &[usize],
    ...
    if scans.is_empty() || remaining_cells == 0 {
        return None;
    }
    ...
    for scan in &scan_source {
        let (scan_remaining, scan_unknown) = scan_adjusted_count(grid, *scan);
        if scan_unknown > 0 {
            constraints.push((*scan, scan_remaining, scan_unknown));
        }
    }
    ...
    for len in 1..remaining.len() {
        ...
        for r in 0..n {
            for c in 0..=n - len {
                if placement_is_legal(grid, shot, r, c, 0, 1, len) {
                    legal_weight_sums[len] += placement_scan_feasibility_weight(grid, r, c, 0, 1, len, &constraints);
                }
            }
        }
        for r in 0..=n - len {
            for c in 0..n {
                if placement_is_legal(grid, shot, r, c, 1, 0, len) {
                    legal_weight_sums[len] += placement_scan_feasibility_weight(grid, r, c, 1, 0, len, &constraints);
                }
            }
        }
    }
    ...
    for len in 1..remaining.len() {
        let ships = remaining[len];
        if ships == 0 || len > n || legal_weight_sums[len] <= 0.0 {
            continue;
        }
        let placement_mass = ships as f64 * len as f64 / legal_weight_sums[len];
        for r in 0..n {
            for c in 0..=n - len {
                if placement_is_legal(grid, shot, r, c, 0, 1, len) {
                    let weight = placement_scan_feasibility_weight(grid, r, c, 0, 1, len, &constraints);
                    if weight <= 0.0 {
                        continue;
                    }
                    for k in 0..len {
                        prob[r][c + k] += placement_mass * weight;
                    }
                }
            }
        }
        for r in 0..=n - len {
            for c in 0..n {
                if placement_is_legal(grid, shot, r, c, 1, 0, len) {
                    let weight = placement_scan_feasibility_weight(grid, r, c, 1, 0, len, &constraints);
                    if weight <= 0.0 {
                        continue;
                    }
                    for k in 0..len {
                        prob[r + k][c] += placement_mass * weight;
                    }
                }
            }
        }
...
fn best_hunt_cell(
    ...
    let scan_fit_prob = build_scan_feasibility_probabilities(n, grid, shot, remaining, scans, remaining_cells);
    let mut best = None;
    ...
    for r in 0..n {
        for c in 0..n {
            ...
            *rng = rng.wrapping_mul(6364136223846793005).wrapping_add(1442695040888963407);
            ...
            let hunt_prob = if let Some(scan_fit) = &scan_fit_prob {
                0.85 * prob[r][c] + 0.15 * scan_fit[r][c]
            } else {
                prob[r][c]
            };
            let score = hunt_prob * density_scale + center_bias + jitter;
            if score > best_score {
                best_score = score;
                best = Some((r, c));
            }
