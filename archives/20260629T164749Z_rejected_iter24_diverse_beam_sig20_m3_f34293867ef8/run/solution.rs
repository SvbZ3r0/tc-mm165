...
const BEAM_DIVERSITY_CELLS: usize = 20;
const BEAM_MAX_STATES_PER_SIGNATURE: usize = 3;
...
fn beam_diversity_signature(bits: &[u64], n: usize, diversity_cells: &[(usize, usize)]) -> u64 {
    let mut signature = 0u64;
    for (idx, &(r, c)) in diversity_cells.iter().take(64).enumerate() {
        if bit_has(bits, r * n + c) {
            signature |= 1u64 << idx;
        }
    }
    signature
...
fn beam_fleet_posterior(
    ...
    for &len in &ship_lengths {
        ...
        let mut signature_counts: Vec<(u64, usize)> = Vec::new();
        let mut diversified_states: Vec<BeamState> = Vec::new();
        for state in next_states {
            let signature = beam_diversity_signature(&state.bits, n, diversity_cells);
            let mut found = false;
            let mut allow = false;
            for (known_signature, count) in signature_counts.iter_mut() {
                if *known_signature == signature {
                    found = true;
                    if *count < BEAM_MAX_STATES_PER_SIGNATURE {
                        *count += 1;
                        allow = true;
                    }
                    break;
            ...
            if !found {
                signature_counts.push((signature, 1));
                allow = true;
            }
            if allow {
                diversified_states.push(state);
                if diversified_states.len() >= beam_width {
                    break;
                }
            }
        }
        states = diversified_states;
    }
...
fn best_hunt_cell(
    ...
    let mut diversity_candidates: Vec<(f64, usize, usize)> = Vec::new();
    for rr in 0..n {
        for cc in 0..n {
            if !shot[rr][cc] && grid[rr][cc] == Cell::Unknown {
                diversity_candidates.push((prob[rr][cc], rr, cc));
            }
        }
    }
    diversity_candidates.sort_by(|a, b| b.0.partial_cmp(&a.0).unwrap());
    let diversity_cells: Vec<(usize, usize)> = diversity_candidates
        .iter()
        .take(BEAM_DIVERSITY_CELLS)
        .map(|&(_, r, c)| (r, c))
        .collect();
    let beam_post = beam_fleet_posterior(n, grid, shot, remaining, scans, remaining_cells, &diversity_cells);
    let max_prob = prob
        .iter()
