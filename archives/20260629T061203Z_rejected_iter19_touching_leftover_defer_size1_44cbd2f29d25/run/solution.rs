...
fn best_leftover_cluster_support(
    n: usize,
    grid: &[Vec<Cell>],
    shot: &[Vec<bool>],
    remaining: &[usize],
    ...
    for &(r, c) in active_hits {
        for (dr, dc) in [(-1isize, 0isize), (1isize, 0isize), (0isize, -1isize), (0isize, 1isize)] {
            ...
            let nr = nr as usize;
            let nc = nc as usize;
            if shot[nr][nc] || grid[nr][nc] != Cell::Unknown {
                continue;
            }
            let support = placement_score_for_hits(n, grid, shot, remaining, active_hits, (nr, nc));
            if support > best {
                best = support;
            }
        }
...
fn should_defer_post_kill_leftover(
    n: usize,
    grid: &[Vec<Cell>],
    shot: &[Vec<bool>],
    remaining: &[usize],
    ...
    let (r, c) = cluster[0];
    let mut touches_killed = false;
    for (dr, dc) in [(-1isize, 0isize), (1isize, 0isize), (0isize, -1isize), (0isize, 1isize)] {
        let nr = r as isize + dr;
        let nc = c as isize + dc;
        if inside(n, nr, nc) && killed[nr as usize][nc as usize] {
            touches_killed = true;
            break;
        }
    }

    touches_killed && best_leftover_cluster_support(n, grid, shot, remaining, cluster) <= 0.0
}
...
fn main() {
    ...
    let mut deferred_clusters: Vec<Vec<(usize, usize)>> = Vec::new();
    ...
    while alive > 0 {
        ...
        match result.as_str() {
                ...
                let inferred_len = if let Some(cluster_idx) = target_cluster {
                    if cluster_idx < active_clusters.len() {
                        ...
                        let leftovers: Vec<(usize, usize)> = killed_cluster
                            ...
                            .collect();
                        for cluster in split_hit_clusters(n, leftovers) {
                            if should_defer_post_kill_leftover(
                                n,
                                &grid,
                                &shot,
                                &remaining,
                                ...
                                deferred_clusters.push(cluster);
                            } else {
            ...
            if target_cluster.is_none() && !deferred_clusters.is_empty() {
                active_clusters.extend(deferred_clusters.drain(..));
            }
