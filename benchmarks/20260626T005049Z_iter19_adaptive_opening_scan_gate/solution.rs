...
fn scan_cost(n: usize, p: f64, scan: Scan) -> f64 {
    let area = (scan.r2 - scan.r1 + 1) * (scan.c2 - scan.c1 + 1);
    p * ((n * n + 2 - area) as f64).log2()
}

fn should_take_opening_scan(
    n: usize,
    p: f64,
    grid: &[Vec<Cell>],
    scan: Scan,
    ...
    if global_unknown == 0 || remaining_cells == 0 {
        return false;
    }
    ...
    let expected_count = remaining_cells as f64 * scan_unknown as f64 / global_unknown as f64;
    let zero_region_value = (scan_unknown as f64) * (-expected_count).exp();
    let density_value = 0.65 * expected_count.max(1.0).sqrt();
    let estimated_value = zero_region_value + density_value;

    estimated_value > scan_cost(n, p, scan)
}
...
fn main() {
    ...
    while alive > 0 {
        if active_clusters.is_empty() && next_opening_scan < opening_scans.len() {
            ...
            next_opening_scan += 1;
            if should_take_opening_scan(n, p, &grid, scan, remaining_cells) {
