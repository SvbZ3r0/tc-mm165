...
fn best_hunt_cell(
    ...
    scans: &[Scan],
    remaining_cells: usize,
    active_hit_count: usize,
    rng: &mut u64,
) -> Option<(usize, usize)> {
    let remaining_ship_cells: usize = (1..remaining.len()).map(|len| len * remaining[len]).sum();
    let corrupt = remaining_ship_cells != remaining_cells + active_hit_count;
    let prob = if !corrupt {
        build_probabilities(n, grid, shot, remaining, scans, remaining_cells)
    } else {
        vec![vec![0.0f64; n]; n]
    };
    let mut best = None;
    ...
    for r in 0..n {
        for c in 0..n {
            ...
            *rng = rng.wrapping_mul(6364136223846793005).wrapping_add(1442695040888963407);
            ...
            let score = if corrupt {
                density_scale + center_bias + jitter
            } else {
                let density_fallback = if prob[r][c] < 1e-10 { density_scale * 2e-2 } else { 0.0 };
                prob[r][c] * density_scale + density_fallback + center_bias + jitter
            };
            if score > best_score {
                best_score = score;
                best = Some((r, c));
...
fn main() {
    ...
    while alive > 0 {
        ...
        let active_hit_count: usize = active_clusters.iter().map(|c| c.len()).sum();
        let target = best_chase_cell(n, &grid, &shot, &active_clusters, &remaining, &mut rng)
            .map(|(cell, cluster_idx)| (cell, Some(cluster_idx)))
            .or_else(|| best_hunt_cell(n, &grid, &shot, &remaining, &scans, remaining_cells, active_hit_count, &mut rng).map(|cell| (cell, None)))
