...
fn infer_killed_hits(
    ...
    // natural span = longest contiguous hit run through kill cell in best direction
    let natural_span_len = {
        let mut row_span = 1usize;
        let mut c = last.1 as isize - 1;
        while c >= 0 && hit_set[last.0 * n + c as usize] { row_span += 1; c -= 1; }
        c = last.1 as isize + 1;
        while c < n as isize && hit_set[last.0 * n + c as usize] { row_span += 1; c += 1; }
        let mut col_span = 1usize;
        let mut r = last.0 as isize - 1;
        while r >= 0 && hit_set[r as usize * n + last.1] { col_span += 1; r -= 1; }
        r = last.0 as isize + 1;
        while r < n as isize && hit_set[r as usize * n + last.1] { col_span += 1; r += 1; }
        row_span.max(col_span)
    };
    // if the natural span has no matching remaining ship, touching ships are present —
    // prefer shortest candidate to avoid over-claiming cells from the adjacent ship
    let has_full_span_candidate = candidates.iter().any(|c| c.len() == natural_span_len);

    candidates.sort();
    candidates.dedup();
    if has_full_span_candidate {
