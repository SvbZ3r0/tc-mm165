...
fn build_probabilities(
    ...
    if remaining_cells > 0 && global_density > 0.0 {
        for scan in &scan_source {
            ...
            let expected_fraction = scan_unknown as f64 / global_unknown.max(1) as f64;
            scan_constraints.push((*scan, scan_remaining, expected_fraction, ratio));
        }
    }
...
fn placement_scan_weight(
    ...
    dc: usize,
    len: usize,
    constraints: &[(Scan, usize, f64, f64)],
) -> f64 {
    ...
    for &(scan, scan_remaining, expected_fraction, ratio) in constraints {
        ...
        if scan_remaining == 0 && overlap > 0 {
            return 0.0;
        }

        let expected_overlap = len as f64 * expected_fraction;
        let residual = overlap as f64 - expected_overlap;
        let evidence = (ratio.ln() * residual * 0.75).clamp(-1.50, 1.50);
        weight *= evidence.exp();
    }

    weight.clamp(0.12, 5.00)
}
