...
const ENDGAME_MAX_SHIPS: usize = 5;
const ENDGAME_MAX_SOLUTIONS: usize = 12000;
const ENDGAME_MAX_NODES: usize = 250000;
...
#[derive(Clone)]
struct EndgamePlacement {
    cells: Vec<(usize, usize)>,
    hit_count: usize,
    weight: f64,
}

fn build_endgame_probabilities(
    n: usize,
    grid: &[Vec<Cell>],
    shot: &[Vec<bool>],
    remaining: &[usize],
    ...
    let ship_count: usize = remaining.iter().sum();
    if ship_count == 0 || ship_count > ENDGAME_MAX_SHIPS {
        return None;
    }
    if remaining_cells > 18 && ship_count > 4 {
        return None;
    }
    ...
    let mut placements_by_ship: Vec<Vec<EndgamePlacement>> = Vec::new();
    for &len in &ship_lens {
        if len == 0 || len > n {
            return None;
        }
        ...
        for r in 0..n {
            for c in 0..=n - len {
                if placement_is_legal(grid, shot, r, c, 0, 1, len) {
                    let weight = placement_scan_weight(r, c, 0, 1, len, &scan_constraints);
                    if weight <= 0.0 {
                        continue;
                    }
                    ...
                    for k in 0..len {
                        let cell = (r, c + k);
                        if hit_set[cell.0 * n + cell.1] {
                            hit_count += 1;
                        }
                        cells.push(cell);
                    }
                    placements.push(EndgamePlacement { cells, hit_count, weight });
                }
            }
        }
        ...
        for r in 0..=n - len {
            for c in 0..n {
                if placement_is_legal(grid, shot, r, c, 1, 0, len) {
                    let weight = placement_scan_weight(r, c, 1, 0, len, &scan_constraints);
                    if weight <= 0.0 {
                        continue;
                    }
                    let mut cells = Vec::with_capacity(len);
        ...
        placements_by_ship.push(placements);
    }

    placements_by_ship.sort_by_key(|placements| placements.len());

    let mut prob = vec![vec![0.0f64; n]; n];
    let mut occupied = vec![false; n * n];
    let mut selected: Vec<usize> = Vec::with_capacity(remaining_cells);
    let mut solution_count = 0usize;
    let mut node_count = 0usize;

    fn search_endgame(
        idx: usize,
        placements_by_ship: &[Vec<EndgamePlacement>],
        n: usize,
        hit_set: &[bool],
        hit_total: usize,
        covered_hits: usize,
        ...
        *node_count += 1;
        if *node_count > ENDGAME_MAX_NODES {
            return false;
        }
        if idx == placements_by_ship.len() {
            if covered_hits != hit_total {
                return true;
            }
            for &cell in selected.iter() {
                prob[cell / n][cell % n] += solution_weight;
            }
            *solution_count += 1;
            return *solution_count < ENDGAME_MAX_SOLUTIONS;
        }

        for placement in &placements_by_ship[idx] {
            ...
            let keep_going = search_endgame(
                idx + 1,
                placements_by_ship,
                n,
                hit_set,
                hit_total,
                covered_hits + added_hits,
                solution_weight * placement.weight,
                occupied,
                selected,
                prob,
                solution_count,
            ...
            for cell in selected.drain(old_len..) {
                occupied[cell] = false;
            }
            if !keep_going {
                return false;
...
fn best_hunt_cell(
    ...
    let prob = build_endgame_probabilities(n, grid, shot, remaining, scans, remaining_cells)
        .unwrap_or_else(|| build_probabilities(n, grid, shot, remaining, scans, remaining_cells));
    let mut best = None;
...
fn infer_killed_hits(
    ...
    if best.is_empty() {
        longest_killed_segment(n, active_hits, last)
    } else {
        best
    }
}
            ...
            "KILL" => {
                ...
                let inferred_len = if let Some(cluster_idx) = target_cluster {
                    if cluster_idx < active_clusters.len() {
                        ...
                        let killed_hits = infer_killed_hits(n, &remaining, &killed_cluster, (r, c));
