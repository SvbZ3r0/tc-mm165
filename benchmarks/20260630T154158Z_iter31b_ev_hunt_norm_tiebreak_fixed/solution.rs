...
const EV_RERANK_TOP_K: usize = 12;
const EV_INFO_WEIGHT: f64 = 0.025;
const EV_FALLBACK_PRIOR: f64 = 2e-2;
const EV_DENSITY_SCALE_MIN: f64 = 0.20;
const EV_DENSITY_SCALE_MAX: f64 = 3.00;
const EV_ZERO_PROB_EPS: f64 = 1e-10;
const EV_JITTER_SCALE: f64 = 1e-12;
const EV_PLACEMENT_EPS: f64 = 1e-9;
...
fn hunt_density_scale_for_cell(
    n: usize,
    scans: &[Scan],
    remaining_cells: usize,
    r: usize,
    ...
    let global_density = (remaining_cells as f64 / (n * n) as f64).max(EV_PLACEMENT_EPS);
    for scan in scans {
        if r < scan.r1 || r > scan.r2 || c < scan.c1 || c > scan.c2 {
            continue;
        }
        let area = (scan.r2 - scan.r1 + 1) * (scan.c2 - scan.c1 + 1);
        let local_density = scan.count as f64 / area as f64;
        let ratio = (local_density / global_density).clamp(0.25, 2.50);
        density_scale *= ratio;
    }
    density_scale.clamp(EV_DENSITY_SCALE_MIN, EV_DENSITY_SCALE_MAX)
}

fn placement_contains_cell(r0: usize, c0: usize, horizontal: bool, len: usize, r: usize, c: usize) -> bool {
    if horizontal {
        r == r0 && c >= c0 && c < c0 + len
    } else {
        c == c0 && r >= r0 && r < r0 + len
...
fn one_step_shot_information_value(
    n: usize,
    grid: &[Vec<Cell>],
    shot: &[Vec<bool>],
    remaining: &[usize],
    ...
    let mut hit_cover = 0.0f64;
    for len in 1..remaining.len() {
        let copies = remaining[len];
        if copies == 0 || len > n {
            continue;
        }
        let ship_weight = copies as f64;
        for horizontal in [true, false] {
            let max_r = if horizontal { n } else { n + 1 - len };
            let max_c = if horizontal { n + 1 - len } else { n };
            for r0 in 0..max_r {
                for c0 in 0..max_c {
                    if !placement_is_legal(grid, shot, r0, c0, if horizontal { 0 } else { 1 }, if horizontal { 1 } else { 0 }, len) {
                        continue;
                    }
                    total += ship_weight;
                    if placement_contains_cell(r0, c0, horizontal, len, target_r, target_c) {
                        hit_cover += ship_weight;
                    }
                }
            }
        }
    ...
    if total <= EV_PLACEMENT_EPS {
        return 0.0;
    }

    let p_hit = (hit_cover / total).clamp(0.0, 1.0);
    let p_miss = 1.0 - p_hit;
    let miss_eliminated = hit_cover;
    let hit_eliminated = total - hit_cover;

    let miss_info = if p_miss > EV_PLACEMENT_EPS {
        (total / (total - miss_eliminated).max(EV_PLACEMENT_EPS)).ln()
    } else {
        0.0
    };
    let hit_info = if p_hit > EV_PLACEMENT_EPS {
        (total / (total - hit_eliminated).max(EV_PLACEMENT_EPS)).ln()
    } else {
        0.0
    };

    p_miss * miss_info + p_hit * hit_info
}
...
fn best_hunt_cell(
    ...
    let mut fallback_best: Option<(usize, usize)> = None;
    ...
    for r in 0..n {
        for c in 0..n {
            ...
            if fallback_best.is_none() {
                fallback_best = Some((r, c));
            }

            let center = (n as f64 - 1.0) / 2.0;
            let dr = r as f64 - center;
            let dc = c as f64 - center;
            let center_bias = 1e-6 / (1.0 + dr * dr + dc * dc);
            let density_scale = hunt_density_scale_for_cell(n, scans, remaining_cells, r, c);
            *rng = rng.wrapping_mul(6364136223846793005).wrapping_add(1442695040888963407);
            let jitter = ((*rng >> 32) as f64) * EV_JITTER_SCALE;
            let density_fallback = if prob[r][c] < EV_ZERO_PROB_EPS {
                density_scale * EV_FALLBACK_PRIOR
            } else {
                0.0
            };
            let heat_score = prob[r][c] * density_scale + density_fallback + center_bias + jitter;
            candidates.push((heat_score, prob[r][c], r, c));
        }
    }
    ...
    candidates.truncate(EV_RERANK_TOP_K.min(candidates.len()));

    let enriched: Vec<(f64, f64, usize, usize, f64)> = candidates
        .iter()
        .map(|&(heat_score, hit_prob, r, c)| {
            let info_value = one_step_shot_information_value(n, grid, shot, remaining, r, c);
            (heat_score, hit_prob, r, c, info_value)
        })
        .collect();
    let min_info = enriched
        .iter()
        .map(|candidate| candidate.4)
        .fold(f64::INFINITY, f64::min);
    let max_info = enriched
        .iter()
        .map(|candidate| candidate.4)
        .fold(f64::NEG_INFINITY, f64::max);
    let info_range = (max_info - min_info).max(EV_PLACEMENT_EPS);

    let mut best = enriched[0];
    let mut best_ev = f64::NEG_INFINITY;
    for &(heat_score, hit_prob, r, c, info_value) in &enriched {
        let info_norm = if max_info > min_info {
            (info_value - min_info) / info_range
        } else {
            0.5
        };
        let ev_score = heat_score * (1.0 + EV_INFO_WEIGHT * (info_norm - 0.5));
        if ev_score > best_ev {
            best_ev = ev_score;
            best = (heat_score, hit_prob, r, c, info_value);
        }
    }
