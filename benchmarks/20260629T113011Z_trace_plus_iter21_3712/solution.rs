...
fn best_hunt_cell(
    ...
    let mut diag_candidates: Vec<(usize, usize, f64, f64, f64, f64)> = Vec::new();
...
fn chase_cell(
    ...
    Some(((candidates[0].0, candidates[0].1), candidates[0].2))
}
...
fn main() {
    ...
    let mut diag: Option<File> = std::env::var("BS_DIAG").ok().and_then(|p| File::create(p).ok());
    ...
    if let Some(file) = diag.as_mut() { let _ = writeln!(file, "GAME n={} p={:.6} remaining_cells={} remaining={:?}", n, p, remaining_cells, remaining); }
    let mut alive = s;
    ...
    while alive > 0 {
        ...
        if active_clusters.is_empty() && next_opening_scan < opening_scans.len() {
            ...
            next_opening_scan += 1;
            diag_log(&mut diag, &format!("turn={} mode=SCAN region={},{},{},{}", turn, scan.r1, scan.c1, scan.r2, scan.c2));
            println!("SCAN {} {} {} {}", scan.r1, scan.c1, scan.r2, scan.c2);
            stdout.flush().unwrap();
            scan.count = read_line(&mut reader);
            diag_log(&mut diag, &format!("turn={} result=SCAN count={}", turn, scan.count));
        ...
        let target = best_chase_cell(n, &grid, &shot, &active_clusters, &remaining, &mut rng, turn, &mut diag)
            .map(|(cell, cluster_idx)| (cell, Some(cluster_idx)))
            .or_else(|| best_hunt_cell(n, &grid, &shot, &remaining, &scans, remaining_cells, &mut rng, turn, &mut diag).map(|cell| (cell, None)))
        ...
        if let Some(file) = diag.as_mut() { let _ = writeln!(file, "RESULT turn={} cell={},{} result={} remaining_cells={} active_clusters={}", turn, r, c, result, remaining_cells, active_clusters.len()); }
