...
fn intersects_bits(a: &[u64], b: &[u64]) -> bool {
...
fn build_fleet_placements(n: usize, len: usize, grid: &[Vec<Cell>], shot: &[Vec<bool>]) -> Vec<FleetPlacement> {
    let words = (n * n + 63) / 64;
    ...
    for r in 0..n {
        for c in 0..=n - len {
            ...
            for k in 0..len {
                ...
                cells.push((rr, cc));
                set_bit(&mut bits, rr * n + cc);
            }
            if ok {
                placements.push(FleetPlacement { cells, bits });
            }
        }
    }
...
fn shuffle_indices(len: usize, rng: &mut u64) -> Vec<usize> {
...
fn sampled_fleet_posterior(
    ...
    if known_hits.is_empty() && scans.is_empty() && remaining_cells * 4 > n * n {
        return None;
    }
    ...
    let words = (n * n + 63) / 64;
    let mut by_len: Vec<Vec<FleetPlacement>> = vec![Vec::new(); remaining.len()];
    for len in 1..remaining.len() {
        if remaining[len] > 0 {
            by_len[len] = build_fleet_placements(n, len, grid, shot);
    ...
    for (idx, scan) in scans.iter().enumerate() {
        ...
        dead_in_scan[idx] = dead;
    }

    let target_samples = if remaining_cells <= n { 160 } else { 96 };
    let max_attempts = target_samples * 12;
    let mut posterior = vec![vec![0.0; n]; n];
    let mut accepted = 0usize;

    'attempts: for _ in 0..max_attempts {
        let mut occupied = vec![0u64; words];
        let mut chosen: Vec<&FleetPlacement> = Vec::with_capacity(ship_lengths.len());

        for &len in &ship_lengths {
            let placements = &by_len[len];
            let order = shuffle_indices(placements.len(), rng);
            let mut picked = None;
            for idx in order.into_iter().take(28) {
                let placement = &placements[idx];
                if !intersects_bits(&occupied, &placement.bits) {
                    picked = Some(placement);
                    break;
                }
            }
            if let Some(placement) = picked {
                or_bits(&mut occupied, &placement.bits);
        ...
        for &(r, c) in &known_hits {
            if !has_bit(&occupied, r * n + c) {
                continue 'attempts;
            }
        }

        if !scans.is_empty() {
            let mut scan_counts = dead_in_scan.clone();
            for placement in &chosen {
                for &(r, c) in &placement.cells {
                    for (idx, scan) in scans.iter().enumerate() {
                        if scan.r1 <= r && r <= scan.r2 && scan.c1 <= c && c <= scan.c2 {
                            scan_counts[idx] += 1;
                            if scan_counts[idx] > scan.count {
                                continue 'attempts;
                            }
                        }
                    }
...
fn best_hunt_cell(
    ...
    let posterior = sampled_fleet_posterior(n, grid, shot, remaining, scans, remaining_cells, rng);
    let max_prob = prob
        .iter()
        .flat_map(|row| row.iter())
        .fold(0.0f64, |a, &b| a.max(b));
    let max_post = posterior
        .as_ref()
    ...
    for r in 0..n {
        for c in 0..n {
            ...
            *rng = rng.wrapping_mul(6364136223846793005).wrapping_add(1442695040888963407);
            ...
            let posterior_prob = if let Some((post, _)) = posterior.as_ref() {
                    if max_post > 0.0 && max_prob > 0.0 {
                        Some(post[r][c] / max_post * max_prob)
                    } else {
                        None
                    }
                } else {
