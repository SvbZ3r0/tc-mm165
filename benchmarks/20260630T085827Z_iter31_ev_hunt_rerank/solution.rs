...
const EV_INFO_WEIGHT: f64 = 0.06;
const EV_HIT_INFO_WEIGHT: f64 = 0.04;
...
fn one_step_shot_information_value(
    ...
    let mut miss_eliminated = 0.0f64;
    let mut hit_eliminated = 0.0f64;
    ...
    miss_eliminated = hit_cover;
    hit_eliminated = total - hit_cover;
...
fn best_hunt_cell(
    ...
    for &(heat_score, hit_prob, r, c) in &candidates {
        let info_value = one_step_shot_information_value(n, grid, shot, remaining, r, c);
        let ev_score = heat_score + EV_INFO_WEIGHT * info_value + EV_HIT_INFO_WEIGHT * hit_prob * info_value;
        if ev_score > best_ev {
            best_ev = ev_score;
            best = (heat_score, hit_prob, r, c);
        }
    }
