...
fn best_hunt_cell(
    ...
    let max_beam = beam_post
        ...
        .unwrap_or(0.0);
    if let Some((post, states)) = beam_post.as_ref() {
        let mut heat_best = -1.0f64;
        let mut heat_second = -1.0f64;
        let mut heat_cell = (0usize, 0usize);
        let mut beam_best = -1.0f64;
        let mut beam_second = -1.0f64;
        let mut beam_cell = (0usize, 0usize);
        for rr in 0..n {
            for cc in 0..n {
                ...
                let h = prob[rr][cc];
                if h > heat_best {
                    heat_second = heat_best;
                    heat_best = h;
                    heat_cell = (rr, cc);
                } else if h > heat_second {
                    heat_second = h;
                }
                let b = post[rr][cc];
                if b > beam_best {
                    beam_second = beam_best;
                    beam_best = b;
                    beam_cell = (rr, cc);
                } else if b > beam_second {
                    beam_second = b;
                }
            }
        }
        diag_extra_line(&format!(
            "BEAMSTAT n={} remaining_cells={} states={} heat_cell={},{} heat_best={:.6} heat_second={:.6} beam_cell={},{} beam_best={:.6} beam_second={:.6}",
            n,
            remaining_cells,
            states,
            heat_cell.0,
