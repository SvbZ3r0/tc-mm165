...
fn best_chase_cell(
    ...
    for (idx, cluster) in active_clusters.iter().enumerate() {
        if let Some((r, c)) = chase_cell(n, grid, shot, cluster, remaining, scans, remaining_cells, rng) {
...
fn main() {
    ...
    while alive > 0 {
        ...
        let target = best_chase_cell(n, &grid, &shot, &active_clusters, &remaining, &scans, remaining_cells, &mut rng)
