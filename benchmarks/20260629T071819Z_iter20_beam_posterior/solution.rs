...
fn make_beam_placement(
    n: usize,
    cells: Vec<(usize, usize)>,
    grid: &[Vec<Cell>],
    shot: &[Vec<bool>],
    scans: &[Scan],
) -> Option<BeamPlacement> {
    let words = (n * n + 63) / 64;
    ...
    for &(r, c) in &cells {
        ...
        bit_set(&mut bits, r * n + c);
    }

    prior += hit_count as f64 * 1000.0;
    Some(BeamPlacement { bits, scan_counts, prior })
}
...
fn build_beam_placements(
    ...
    for r in 0..n {
        for c in 0..=n - len {
            let cells: Vec<(usize, usize)> = (0..len).map(|k| (r, c + k)).collect();
            if let Some(p) = make_beam_placement(n, cells, grid, shot, scans) {
                placements.push(p);
            }
        }
    }
    if len > 1 {
        for r in 0..=n - len {
            for c in 0..n {
                let cells: Vec<(usize, usize)> = (0..len).map(|k| (r + k, c)).collect();
                if let Some(p) = make_beam_placement(n, cells, grid, shot, scans) {
                    placements.push(p);
                }
            }
...
fn beam_fleet_posterior(
    ...
    if known_hits.is_empty() && scans.is_empty() {
        return None;
    }
    if known_hits.is_empty() && remaining_cells > n * 2 {
        return None;
    }
