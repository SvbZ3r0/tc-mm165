...
fn scored_kill_candidate(
    ...
    if active.is_empty() {
        return (-200 - 50 * unknown_count, active);
    }

    let min_idx = active_positions.iter().copied().min().unwrap().min(last_idx);
    let max_idx = active_positions.iter().copied().max().unwrap().max(last_idx);
    let contiguous = (min_idx..=max_idx).all(|idx| idx == last_idx || active_positions.contains(&idx));
    let contiguous_bonus = if contiguous { 20 } else { 0 };
    let exact_bonus = if unknown_count == 0 { 40 } else { 0 };
    let score = active.len() as i32 * 100 + contiguous_bonus + exact_bonus - unknown_count * 50;
    (score, active)
}
...
fn infer_killed_hits(
    ...
    let exact = exact_killed_placement(n, remaining, active_hits, last);
    let fallback = if exact.is_empty() {
    ...
    let mut best_hits: Vec<(usize, usize)> = Vec::new();
    ...
    for len in 1..remaining.len() {
        ...
        for start_c in min_c..=max_c {
            ...
            for k in 0..len {
                ...
                if cell == last {
                    last_idx = k;
                } else if grid[cell.0][cell.1] == Cell::Miss || grid[cell.0][cell.1] == Cell::Dead {
                    ok = false;
                    break;
                }
                cells.push(cell);
            ...
            if ok {
                let (score, hits) = scored_kill_candidate(&cells, last_idx, &hit_set, n);
                if score > best_score || (score == best_score && hits.len() > best_hits.len()) {
                    best_score = score;
                    best_hits = hits;
                }
            }
            ...
            "KILL" => {
                ...
                let inferred_len = if let Some(cluster_idx) = target_cluster {
                    if cluster_idx < active_clusters.len() {
                        ...
                        let killed_hits = infer_killed_hits(n, &grid, &remaining, &killed_cluster, (r, c));
