...
fn max_remaining_length(remaining: &[usize]) -> usize {
    (1..remaining.len())
        .rev()
        .find(|&len| remaining[len] > 0)
        .unwrap_or(0)
}

fn has_remaining_length_at_least(remaining: &[usize], len: usize) -> bool {
    (len..remaining.len()).any(|l| remaining[l] > 0)
}

fn segment_len_with_candidate(active_hits: &[(usize, usize)], candidate: (usize, usize)) -> usize {
    let mut min_r = candidate.0;
    let mut max_r = candidate.0;
    let mut min_c = candidate.1;
    let mut max_c = candidate.1;
...
fn add_chase_candidate(
    ...
    let implied_len = segment_len_with_candidate(active_hits, (r, c));
    let max_len = max_remaining_length(remaining);
    if implied_len > max_len || !has_remaining_length_at_least(remaining, implied_len) {
        return;
    }
    ...
    let placement_bonus = if active_hits.len() == 1 {
        placement_score * 0.25
    } else {
        placement_score
    };
    candidates.push((r, c, base + placement_bonus + prob[r][c]));
}
