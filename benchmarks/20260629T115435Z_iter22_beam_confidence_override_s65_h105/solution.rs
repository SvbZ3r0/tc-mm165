...
const BEAM_OVERRIDE_MAX_STATES: usize = 65;
const HEATMAP_DECISIVE_RATIO: f64 = 1.05;
const BEAM_MIN_VALID_STATES: usize = 8;
const HEATMAP_SECOND_EPS: f64 = 1.0e-9;
...
fn best_hunt_cell(
    ...
    let max_beam = beam_post
        ...
        .unwrap_or(0.0);
    let mut heat_best = 0.0f64;
    let mut heat_second = 0.0f64;
    for rr in 0..n {
        for cc in 0..n {
            if shot[rr][cc] || grid[rr][cc] != Cell::Unknown {
                continue;
            }
            let value = prob[rr][cc];
            if value > heat_best {
                heat_second = heat_best;
                heat_best = value;
            } else if value > heat_second {
                heat_second = value;
            }
        }
    }
    let heat_ratio = heat_best / heat_second.max(HEATMAP_SECOND_EPS);
    let use_beam_override = beam_post
        .as_ref()
        .map(|(_, states)| *states <= BEAM_OVERRIDE_MAX_STATES && heat_ratio <= HEATMAP_DECISIVE_RATIO)
        .unwrap_or(false);
    let mut best = None;
        ...
        for c in 0..n {
            ...
            *rng = rng.wrapping_mul(6364136223846793005).wrapping_add(1442695040888963407);
            ...
            let posterior_prob = if let Some((post, states)) = beam_post.as_ref() {
                ...
                let score = if use_beam_override {
                    if let Some(post_score) = posterior_prob {
                        post_score + center_bias + jitter
                    } else {
                        prob[r][c] * density_scale + center_bias + jitter
                    }
                } else {
                    prob[r][c] * density_scale + center_bias + jitter
                };
