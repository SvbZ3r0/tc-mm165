...
fn infer_killed_hits(
    ...
    let candidates = [Vec::new(), row_segment, col_segment];
    candidates
        .into_iter()
        .max_by_key(|segment| {
            let inferred_len = segment.len() + 1;
            let length_available = inferred_len < remaining.len() && remaining[inferred_len] > 0;
            (length_available, segment.len())
        })
        .unwrap_or_default()
}
