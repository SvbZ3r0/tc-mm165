...
fn push_single_hit_axis_candidate(
    n: usize,
    grid: &[Vec<Cell>],
    shot: &[Vec<bool>],
    active_hits: &[(usize, usize)],
    ...
    if placement_score <= 0.0 {
        return None;
    }
    let placement_bonus = placement_score * 0.25;
    Some((rr, cc, placement_bonus + prob[rr][cc]))
}
...
fn chase_cell(
    ...
    if active_hits.len() == 1 {
        let (r, c) = active_hits[0];
        let horizontal = [
            push_single_hit_axis_candidate(n, grid, shot, active_hits, remaining, &prob, r as isize, c as isize - 1),
            push_single_hit_axis_candidate(n, grid, shot, active_hits, remaining, &prob, r as isize, c as isize + 1),
        ];
        let vertical = [
            push_single_hit_axis_candidate(n, grid, shot, active_hits, remaining, &prob, r as isize - 1, c as isize),
            push_single_hit_axis_candidate(n, grid, shot, active_hits, remaining, &prob, r as isize + 1, c as isize),
        ];

        let horizontal_score: f64 = horizontal.iter().flatten().map(|&(_, _, score)| score).sum();
        let vertical_score: f64 = vertical.iter().flatten().map(|&(_, _, score)| score).sum();
        let chosen = if horizontal_score >= vertical_score { horizontal } else { vertical };

        for item in chosen.into_iter().flatten() {
            candidates.push((item.0, item.1, 5000.0 + item.2));
        }

        if candidates.is_empty() {
            for item in horizontal.into_iter().chain(vertical.into_iter()).flatten() {
                candidates.push((item.0, item.1, 5000.0 + item.2));
            }
        }
    } else {
            ...
            for nc in [min_c as isize - 1, max_c as isize + 1] {
                if inside(n, r as isize, nc) {
                    ...
                    if !shot[r][c] && grid[r][c] == Cell::Unknown {
                        ...
                        let placement_bonus = placement_score;
